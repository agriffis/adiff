...
class TestCase(unittest.TestCase):
    def assertEqual(self, first, second, message=None):
        ...
        if (
            ...
            message = "".join(
                difflib.ndiff(first.splitlines(True), second.splitlines(True))
            )
...
class UtilTest(TestCase):
    ...
    def test_isplit(self):
        ...
        self.assertListsEqual(list(isplit(r"\s+", "")), [""])
        self.assertListsEqual(list(isplit(r"\s+", "x")), ["x"])
        self.assertListsEqual(list(isplit(r"\s+", " ")), ["", " ", ""])
        self.assertListsEqual(
            list(isplit(r"\s+", "foo  bar baz")), ["foo", "  ", "bar", " ", "baz"]
        )
        self.assertListsEqual(
        ...
        self.assertListsEqual(
            list(isplit(r" ", "foo  bar baz")), ["foo", " ", "", " ", "bar", " ", "baz"]
        )
        self.assertListsEqual(
            list(isplit(r"a", "foo  bar baz")), ["foo  b", "a", "r b", "a", "z"]
        )

    def test_countlines(self):
        from adiff import countlines as cl

        self.assertListsEqual([cl(""), cl("\n"), cl("\n\n")], [0, 1, 2])
        self.assertListsEqual(
            [cl("foo"), cl("foo\n"), cl("foo\nbar"), cl("foo\nbar\n")], [1, 1, 2, 2]
        )
